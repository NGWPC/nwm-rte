...
##     * A typical choice for this is `${THIS_SCRIPTS_GRANDPARENT_DIR}`, which is equivalent to `"${HOME}/ngwpc"` if you run this from `"${HOME}/ngwpc/nwm-rte"` but another location such as `"${HOME}/ngwpc__rte"` could be used if wanting to isolate the RTE from other work.

...
set -x
	...
	#-v "${MNT__RUN_NGEN__HOST}/data/raw_input:/ngen-app/data/raw_input" \
        #-v "/contrib/lfs/h1/ops/prod/com:/ngen-app/data/raw_input" \
	#-v "/contrib/lfs/h1/owp/ptmp/Zhengtao.Cui/test/tmp/scratch:/ngen-app/data/scratch" \
    ...
    time sudo docker run --entrypoint "${entrypoint}" ${interactive_terminal} \
        ...
        -v "/contrib/lfs/h1/ops/prod/com:/lfs/h1/ops/prod/com" \
	-v "${MNT__RUN_NGEN__HOST}:/ngwpc/tmp" \
	-v "${MNT__RUN_NGEN__HOST}/scratch:/lfs/h1/owp/ptmp/test/tmp/scratch" \
	-v "${MNT__RUN_NGEN__HOST}/esmf_mesh:/ngen-app/data/esmf_mesh" \
	-v "${MNT__MODULE_PARAM_FILES_DIR__HOST}:/ngwpc/parm" \
        \
